...
import copy
def record_parameters(model):
    return {name: copy.deepcopy(param) for name, param in model.named_parameters()}
def find_unchanged_parameters(initial_params, current_params):
    unchanged_params = {}
    for name in initial_params:
        if torch.equal(initial_params[name], current_params[name]):
            unchanged_params[name] = initial_params[name]
    return unchanged_params

def create_model(args, num_labels, model_class_fn):
  # Prepare model
  rank = getattr(args, 'rank', 0)
  init_model = args.init_model if rank<1 else None
  model = StudentModel.load_model( "deberta-v3-xsmall", args.model_config, num_labels=num_labels, \
      drop_out=args.cls_drop_out,cluster_path=args.cluster_path, only_return_hidden=False )
  
  #print("---------------------------")
  if args.student_ckpt_path is not None:
    print("Load ckpt from", args.student_ckpt_path)
    bin_file_path = args.student_ckpt_path
...
def main(args):
  ...
  #<bound method NNModule.load_model of <class 'DeBERTa.apps.models.sequence_classification.SequenceClassificationModel'>>
...
def build_argument_parser():
  ...
  parser.add_argument('--cluster_path',
            ...
            type=str,
            help="Cluster Path")
  parser.add_argument('--student_ckpt_path',
            default=None,
            type=str,
