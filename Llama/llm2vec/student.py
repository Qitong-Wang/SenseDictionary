...
#import experiments.mteb_eval as me
...
class Student(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model.layers = model.layers[:me.student_layers]
        ...
        if os.path.exists(f"{config_addr}/student_config.json"):
            with open(f"{config_addr}/student_config.json", "r") as fIn:
                student_config = json.load(fIn)
            config.update(student_config)
    ...
    def forward(self, sentence_feature: Dict[str, Tensor]):
        ...
        if me.run_option == "replace":
            ...
            #sense_emb = torch.round(reps.last_hidden_state.clone())
        ...
        elif me.run_option == "round":
            sense_d =  torch.round(reps.last_hidden_state,decimals = me.d)
            return self.get_pooling(sentence_feature,sense_d)
    ...
    def save(self, output_path, merge_before_save=False, save_config=True):
        if merge_before_save and isinstance(self.model, PeftModel):
            ...
            # Fixes the issue of saving - https://huggingface.co/McGill-NLP/Student-Mistral-7B-Instruct-v2-mntp-unsup-simcse/discussions/1
        ...
        if save_config:
            ...
            with open(f"{output_path}/student_config.json", "w") as fOut:
                json.dump(student_config, fOut, indent=4)
