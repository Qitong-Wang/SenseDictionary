...
import a2_train_multi as me
embedding_dict = dict()

freq_id = []
def replace_sense(input_emb, sense_emb, type, pca=None, pca_emb=None):

    if  torch.is_tensor(sense_emb) and sense_emb.dim() == 1:
        return sense_emb
    if type == "dot":
...
def get_unique_save_path_for_txt(save_path):
...
class Teacher(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if os.path.exists(f"{config_addr}/teacher_config.json"):
            with open(f"{config_addr}/teacher_config.json", "r") as fIn:
                teacher_config = json.load(fIn)
    ...
    def forward(self, sentence_feature: Dict[str, Tensor]):
        ...
        sense_emb =  reps.last_hidden_state.clone().to('cpu')
        
        if me.run_option == "replace" or me.run_option == "teacher_replace":
            print("teacher_replace")
            for i in range(input_ids.shape[0]):
                for j in  range(input_ids.shape[1]):
                    input_id = input_ids[i,j].item()
                    if input_id  in freq_id:
                        continue
                    if sentence_feature["attention_mask"][i,j] == 0:
                        continue 
                    else:
                        if input_id not in me.sense_dict.keys():
                            print("Missing",input_id)
                        else:
                            sense_emb[i,j,:] = replace_sense(sense_emb[i,j,:],me.sense_dict[input_id],type=me.sense_func)
           

            sense_emb = sense_emb.to(reps.last_hidden_state.device)
            return self.get_pooling(sentence_feature,sense_emb)
        elif me.run_option == "cluster":
            # Save embedding dict
            input_ids = sentence_feature["input_ids"]
            for i in range(input_ids.shape[0]):
                for j in  range(input_ids.shape[1]):
                    if sentence_feature["attention_mask"][i,j] == 0:
                        continue 
                    input_id = input_ids[i,j].item()
                    if input_id in embedding_dict.keys():
                        embedding_dict[input_id].append(reps.last_hidden_state[i,j,:].detach().cpu())
                    else:
                        embedding_dict[input_id] = list()
                        embedding_dict[input_id].append(reps.last_hidden_state[i,j,:].detach().cpu())
            
            return self.get_pooling(sentence_feature, reps.last_hidden_state)
        elif  me.run_option == "hidden":
            return (sentence_feature, reps.last_hidden_state)
        else:
            return self.get_pooling(sentence_feature, reps.last_hidden_state)
    ...
    def encode(
        ...
        batch_size: int = 16, # batch_size default: 32
        ...
        if me.run_option == "save_txt":
            processed_text = [self.prepare_for_tokenization(sentence) for sentence in sentences_sorted]
            with open(get_unique_save_path_for_txt(me.save_path), 'w') as handle:
                json.dump(processed_text, handle)
        all_embeddings = []
        ...
        if torch.cuda.device_count() <= 1:
            ...
            with cuda_compatible_multiprocess.Pool(num_proc) as p:
                ...
                for idx, batch in enumerate(sentences_batches):
                    last_loop = False
                    if idx == len(sentences_batches) - 1:
                        last_loop = True
                    results.append(
                        p.apply_async(
                            self._encode,
                            args=(batch, None, convert_to_numpy, True,last_loop),
                            callback=update,
                        )
                    )
    ...
    def save(self, output_path, merge_before_save=False, save_config=True):
        if merge_before_save and isinstance(self.model, PeftModel):
            ...
            # Fixes the issue of saving - https://huggingface.co/McGill-NLP/Teacher-Mistral-7B-Instruct-v2-mntp-unsup-simcse/discussions/1
        ...
        if save_config:
            ...
            with open(f"{output_path}/teacher_config.json", "w") as fOut:
                json.dump(teacher_config, fOut, indent=4)
    ...
    def _encode(
        ...
        if me.run_option == "cluster":
            #if last_loop:
            thread_id = threading.get_ident()
            secure_random = random.SystemRandom()
            random_integer = secure_random.randint(1, 10000)    
            with open(me.save_path[:-4]+str(thread_id)+"_"+str(random_integer)+".pkl", 'wb') as handle:
    ...
    def evaluate_model(
           ...
           # device: Optional[str] = None,
            ...
            #print("Already set device. Should not be called for infer. ./llmv2vec/teacher.py L651")
            """
            Encode a list of sentences to their respective embeddings. The sentences can be a list of strings or a string.
            Args:
                sentences: sentence or sentences to encode.
            ...
            for sentence in sentences:
                ...
                #print(self._convert_to_str(sentence[0], sentence[1]))
            sentences = concatenated_input_texts
            '''
    ...
    def _evaluate(
        ...
        embed_mask = None
        if "embed_mask" in features:
            embed_mask = features.pop("embed_mask")
        reps = self.model(**features)
        ...
        #pool =  self.get_pooling(features, reps.last_hidden_state)
    
        if me.run_option == "replace":
            for i in range(input_ids.shape[0]):
                for j in  range(input_ids.shape[1]):
                    input_id = input_ids[i,j].item()
                    if input_id  in freq_id:
                        continue
                    if features["attention_mask"][i,j] == 0:
                        continue 
                    else:
                        if input_id  in me.sense_dict.keys():
                            sense_emb[i,j,:] = replace_sense(sense_emb[i,j,:],me.sense_dict[input_id],type=me.sense_func)

            sense_emb = sense_emb.to(reps.last_hidden_state.device)
            return (input_ids,features["attention_mask"],sense_emb)
        else:
         return (input_ids, features["attention_mask"]  , reps.last_hidden_state)
